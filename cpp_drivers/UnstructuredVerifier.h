...
private:
  ...
  template <typename Value> void setDefaultPrecision() {
  ...
  std::tuple<bool, value_type>
  compare_below_threshold(value_type expected, value_type actual,
...
public:
  ...
  UnstructuredVerifier(double precision)
      : use_default_precision_(false), precision_(precision) {}

  template <typename Value, int RANK>
  bool compareArrayView(const atlas::array::ArrayView<Value, RANK> &lhs,
                        const atlas::array::ArrayView<Value, RANK> &rhs,
                        int max_erros = 10) {
